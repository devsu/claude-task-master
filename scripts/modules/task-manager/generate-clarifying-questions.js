...
async function generateClarifyingQuestions(options, context = {}) {
	...
	const { id, file, output, tasksData } = options; // Added tasksData
	...
	// Validate input: must provide exactly one of id, file, or tasksData
	if ([id, file, tasksData].filter(Boolean).length !== 1) {
		const errorMessage =
			'Please provide either a --id (task ID) or a --file (PRD path), but not both.';
	...
	if (!projectRoot) {
		const errorMessage =
			'Could not determine project root. Please run `task-master init` first.';
		reportLog(errorMessage, 'error');
		if (outputFormat === 'text') {
			console.error(chalk.red(errorMessage));
		}
	...
	try {
		if (id) {
			...
			// Assuming a tagged structure, we need to find the task in the 'master' tag or a specified tag.
			// For simplicity, this example checks the 'master' tag. A more robust solution would check the current tag.
			const task = allTasksData.master.tasks.find((t) => t.id === taskId);
			...
			reportLog(
				`Analyzing task ${task.id} for clarifying questions...`,
				'info'
			);
		} else if (file) {
			...
			// New condition to handle tasksData passed directly
			const tasksToProcess = tasksData.tasks;
			if (tasksToProcess.length === 0) {
				reportLog(
					'No tasks provided in tasksData for clarifying questions.',
					'warn'
				);
				return {
					success: true,
					data: { filePath: null, message: 'No tasks to clarify.' }
				};
			}
			// Combine relevant info from all tasks into contentToClarify
			contentToClarify = tasksToProcess
				.map(
					(t) =>
			...
			reportLog(
				`Analyzing ${tasksToProcess.length} task(s) for clarifying questions...`,
				'info'
			);
		}
...
Provide the questions in a clear, numbered list format. Do not include any introductory or concluding remarks, just the numbered list of questions.`;
		...
		const outputFilePath = output
			...
			: path.join(taskMasterDocsDir, defaultFileName);
		...
		reportLog(
			chalk.green(
				`Clarifying questions document generated successfully at: ${outputFilePath}`
			),
			'success'
		);
		if (outputFormat === 'text') {
		...
		return {
			success: true,
			data: { filePath: outputFilePath, title: documentTitle }
		};
	} catch (error) {
